...
## Show the game over screen with run summary.
...
func _on_draw() -> void:
	...
	## Title
	var title := "SIGNAL FOUND" if _victory else "SIGNAL LOST"
	var title_color := VICTORY_COLOR if _victory else DEFEAT_COLOR
	var title_size := font.get_string_size(title, HORIZONTAL_ALIGNMENT_CENTER, -1, 24)
	_draw_node.draw_string(font, Vector2((vp_size.x - title_size.x) * 0.5, y), title, HORIZONTAL_ALIGNMENT_CENTER, -1, 24, title_color)
	y += 16
...
func _get_button_at(mouse_pos: Vector2, vp_size: Vector2) -> int:
	...
	var btns_total := btn_w * 2 + btn_gap
	var btn_start_x := (vp_size.x - btns_total) * 0.5
	## Stats height = header(~100) + separator(20) + 6 stats * 22 + 16 + separator + 24
	var btn_y := panel_y + 36 + 16 + 44 + 20 + 6 * 22 + 16 + 1 + 24
